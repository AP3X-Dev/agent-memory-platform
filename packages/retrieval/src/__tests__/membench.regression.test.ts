// MemBench regression gate: AMP must beat the naive/keyword baselines on the
// agent-relevant memory dimensions (conflict, stale, contamination), not just recall.
// This pins the benchmark's discriminating power AND AMP's lead over alternatives.

import { describe, expect, it } from 'vitest';
import { runMemBench } from '../../../../bench/membench/run.js';

describe('MemBench — AMP vs baselines', () => {
  it('AMP scores highest composite and beats keyword on conflict/stale/contamination', async () => {
    const reports = await runMemBench();
    const amp = reports.find((r) => r.adapter === 'AMP')!;
    const keyword = reports.find((r) => r.adapter === 'Keyword(BM25)')!;
    const naive = reports.find((r) => r.adapter === 'NaiveRecency')!;

    // AMP leads overall on the agent-relevant quality dimensions.
    ...
    // AMP's edge is specifically the dimensions a plain lexical memory ignores:
