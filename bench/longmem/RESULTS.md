...
evidence session in its top-k? Deterministic, no LLM. MemBerry's real ranking (MemBench
`AmpAdapter`: BM25 channel → `rrfFusion` + provenance + inferred-supersession) vs baselines.

```
Recall@1 (100 questions, hardest):
system          Recall@1   MRR
MemBerry        0.900      0.900
Keyword(BM25)   0.900      0.900
NaiveRecency    0.040      0.040

Recall@5 (40 questions):  MemBerry 1.00 / BM25 1.00 / NaiveRecency 0.225
```

Per-category Recall@1 (MemBerry vs BM25, identical): knowledge-update 1.00/1.00, multi-session
...
1. **MemBerry's retrieval is strong** — Recall@1 = 0.90 (the gold session is rank-1 90% of the
...
2. **MemBerry exactly ties BM25 — no lift.** Honest reason: the `AmpAdapter` here uses a single
   ...
   "BM25 order." MemBerry's real lift comes from **multi-channel (dense + lexical) RRF fusion**,
   **provenance/temporal weighting**, and **project scoping** — none of which this config
   exercises. So this measures "MemBerry's lexical reranking" (≈ BM25), not MemBerry's full stack.
3. **Where MemBerry's differentiators would show:** (a) a **dense/embedding channel** fused with
...
## Hybrid (dense+lexical RRF) — MemBerry's actual architecture (`hybrid_eval.py`, local MiniLM)

Added a real dense channel (sentence-transformers all-MiniLM-L6-v2) fused with BM25 via RRF
(k=60) — MemBerry's true retrieval design. Recall@1, 100 questions:

```
bm25     0.800
dense    0.720      <- embedding-only LOSES to lexical
hybrid   0.810      <- MemBerry's dense+lexical RRF fusion
...
MemBerry's hybrid fusion is **~tied with BM25 overall (+0.01, within noise)** but **modestly
...
embeddings alone — which validates MemBerry's architecture choice. The lift is real but small,
and the absolute retrieval is strong (R@1 0.80–0.90). NOT a dramatic win, NOT a head-to-head
vs Zep/Letta. The bigger MemBerry differentiators (consolidation, fact supersession, temporal
