...
Load MemBerry memory relevant to the current context.

### Flow

1. **Find MemBerry config** — extract project tag, entities, tags from project CLAUDE.md
2. **Build berry_load call:**
   - User gave a topic -> use as task, match entities from config
   - No topic -> infer from conversation context
3. **Call berry_load:**
   ```
   berry_load(task: "<topic>", entities: [<matching>], tags: ["project:<tag>"], max_tokens: 4000)
   ```
4. **Present results:**
...
1. **Pass the question straight to `berry_ask`:**
   ```
   berry_ask(question: "<the question>", reasoning_level: "medium",
...
- `ask` → "answer this question" (MemBerry reasons and replies, with citations).

---

## remember <what>

Store knowledge to MemBerry.
...
3. **Identify entities** — match against known entities from MemBerry config. Or omit and let auto-extraction handle it.

4. **Check for signals** (optional):
   - Load existing knowledge: `berry_load(task: "<topic>", tags: ["project:<tag>"])`
...
Run a query against the MemBerry knowledge graph.
...
3. **Execute:** `berry_query(query: "<cypher>", limit: 10)`
...
```
berry_memory_read(block: "user", scope: "project:my-project")
```
...
```
// Append text
berry_memory_insert(block: "working_state", scope: "project:my-project",
...
berry_memory_replace(block: "user", scope: "project:my-project",
  old_text: "Prefers tabs", new_text: "Prefers 2-space indent")

// Overwrite entirely
berry_memory_rewrite(block: "current_objective", scope: "project:my-project",
...
berry_memory_promote(block: "working_state", from_tier: "working", to_tier: "core",
...
berry_memory_archive(block: "open_questions", scope: "project:my-project",
  session_id: "session-20260409-143000")
```
...
`berry_load` allocates tokens per tier: core ~15%, working ~10%, facts ~15%, archive ~60%. Blocks exceeding their budget are truncated.
...
```
berry_timeline(entity: "auth-module")
berry_timeline(entity: "auth-module", include_episodes: true, limit: 20)
```
...
```
berry_fact_diff(entity: "auth-module", from: "2026-03-01T00:00:00Z", to: "2026-04-01T00:00:00Z")
```
...
Pass the `temporal` parameter to `berry_load` for time-scoped queries:

```
// Current facts only (default)
berry_load(task: "...", temporal: { time_mode: "current" })

// What was believed at a specific time
berry_load(task: "...", temporal: { time_mode: "historical", as_of: "2026-03-15T00:00:00Z" })

// Facts active during an interval
berry_load(task: "...", temporal: { time_mode: "interval", from: "2026-03-01T00:00:00Z", to: "2026-04-01T00:00:00Z" })

// Full fact evolution including invalidated
berry_load(task: "...", temporal: { time_mode: "evolution", include_invalidated: true })
```

### Entity resolution

Facts use canonical entity resolution. "MemBerry", "memberry", and "memberry-core" all resolve to the same entity if aliases exist. The system resolves via: exact name → case-insensitive → alias match. New name variants are automatically added as aliases.
